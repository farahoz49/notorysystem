 ...
 * - Halkan waxaan ku haynay “content-ka GuriDhisan case” + optional local signatures haddii aad rabto
 *   (laakiin haddii aad hore u leedahay signatureTable global, ka tag includeLocalSignatures=false).
 */
export const buildGuriDhisanDoc = ({
  agreement,
  service,
  formatDate,
  formatCurrency,
...
if (agreementType === "Beec") {
  ...
  leftTitle = hasSellerSigAgent
    ? singleOrPlural(
        ...
        `SAXIIXA WAKIILKA ${maleFemale(leftGender, "ISKA IIBIYAHA", "ISKA IIBISADA")} guriga`,
        `SAXIIXA WAKIILLADA ISKA IIBIYAASHA guriga`
      )
    : singleOrPlural(
        sellersArr.length,
        `SAXIIXA ${maleFemale(leftGender, "ISKA IIBIYAHA", "ISKA IIBISADA")} guriga `,
        `SAXIIXA ISKA IIBIYAASHA GURIGA`
      );
  ...
  rightTitle = hasBuyerSigAgent
    ? singleOrPlural(
        ...
        `SAXIIXA BEEC U AQBALAHA ${maleFemale(rightGender, "IIBSADAHA", "IIBSATADA")} GURIGA`,
        `SAXIIXA BEEC U AQBALAHA IIBSADAYAASHA GURIGA`
      )
    : singleOrPlural(
        buyersArr.length,
        `SAXIIXA ${maleFemale(rightGender, "IIBSADAHA", "IIBSATADA")} GURIGA`,
        `SAXIIXA IIBSADAYAASHA GURIGA`
      );
}
...
========================= */
  ...
  leftTitle = hasSellerSigAgent
    ? singleOrPlural(
        sellerAgentsArr.length,
        `SAXIIXA WAKIILKA ${maleFemale(leftGender, "HIBEYAHA", "HIBEYSADA")} GURIGA`,
        `SAXIIXA WAKIILLADA HIBEYAASHA GURIGA`
      )
    : singleOrPlural(
        sellersArr.length,
        `SAXIIXA ${maleFemale(leftGender, "HIBEYAHA", "HIBEYSADA")} GURIGA`,
        `SAXIIXA HIBEYAASHA GURIGA`
      );
  ...
  rightTitle = hasBuyerSigAgent
    ? singleOrPlural(
        buyerAgentsArr.length,
        `SAXIIXA HIBEYN U AQBALAHA ${maleFemale(rightGender, "QAATAHA", "QAATADA")} GURIGA`,
        `SAXIIXA HIBEYN U AQBALAHA QAATAYAASHA GURIGA`
      )
    : singleOrPlural(
        buyersArr.length,
        `SAXIIXA ${maleFemale(rightGender, "QAATAHA", "QAATADA")} GURIGA`,
        `SAXIIXA QAATAYAASHA GURIGA`
      );
}
...
========================= */
  ...
  leftTitle = hasSellerSigAgent
    ? singleOrPlural(
        sellerAgentsArr.length,
        `SAXIIXA WAKIILKA ${maleFemale(leftGender, "WAAQIFAHA", "WAAQIFADA")} GURIGA`,
        `SAXIIXA WAKIILLADA WAAQIFAYAASHA GURIGA`
      )
    : singleOrPlural(
        sellersArr.length,
        `SAXIIXA ${maleFemale(leftGender, "WAAQIFAHA", "WAAQIFADA")} GURIGA`,
        `SAXIIXA WAAQIFAYAASHA GURIGA`
      );
  ...
  rightTitle = hasBuyerSigAgent
    ? singleOrPlural(
        buyerAgentsArr.length,
        `SAXIIXA WAQAF U AQBALAHA ${maleFemale(rightGender, "QOFKA LOO WAQFAY", "QOFTA LOO WAQFAY")} GURIGA`,
        `SAXIIXA WAQAF U AQBALAHA DADKA LOO WAQFAY GURIGA`
      )
    : singleOrPlural(
        buyersArr.length,
        `SAXIIXA ${maleFemale(rightGender, "QOFKA LOO WAQFAY", "QOFTA LOO WAQFAY")} GURIGA`,
        `SAXIIXA DADKA LOO WAQFAY GURIGA`
      );
}
...
const notarySection = [
  ...
  // ========= RETURN (GuriDhisan main content) =========
  const content = [
      // TITLE
 new Paragraph({
  alignment: AlignmentType.CENTER,
  ...
  children: [
    new TextRun({
      text:
        agreement.agreementType === "Beec"
          ? "UJEEDDO: KALA GADASHO GURI DHISAN"
          : agreement.agreementType === "Hibo"
          ? "UJEEDDO: HIBEYN GURI DHISAN"
          : agreement.agreementType === "Waqaf"
          ? "UJEEDDO: WAQFID GURI DHISAN"
          : "",
      bold: true,
      underline: true,
      size: 24,
    ...
    new Paragraph({
      ...
      children: hasSellerAgent
        ? [
            ...
            new TextRun({ text: `, ${wakaaladText}, kana wakiil ah iska ${T?.seller} guriga `, size: 24 }),
            ...
            ...(agreement?.sellingPrice
              ? [
                  new TextRun({
                    text: ` Waxaan ku gaday anigoo ka wakiil ah ${T?.seller} guriga lacag dhan $${formatCurrency(
                      agreement.sellingPrice
    ...
    new Paragraph({
      ...
      children: hasBuyerAgent
        ? [
            new TextRun({ text: "BEEC U AQBALAHA IIBSADAHA GURIGA", size: 24, underline: true, bold: true }),
            new TextRun({ text: " Anigoo ah ", size: 24 }),
            ...
            new TextRun({
              text:
                ...
                `Sidaasi darteed laga bilaabo taariikhda kor ku xusan, maamulkii iyo manfacii gurigaas ` +
