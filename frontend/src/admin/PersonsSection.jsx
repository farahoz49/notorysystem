...
const PersonsSection = ({
  ...
  const handlePerson = async (operation, personId = null, data = null) => {
    try {
      ...
      // ---------- ADD ----------
      if (operation === "add") {
        // ✅ haddii data leeyahay _id -> waa existing person (keydsan)
        // markaas create ma sameynayno, link kaliya
        if (data?._id) {
          ...
          // optional: if agent -> open link modal for Wakaalad
        ...
        // ✅ haddii uusan _id lahayn -> cusub
        const phone = String(data?.phone || "").trim();
        const docNo = String(data?.documentNumber || "").trim();

        const phoneExists =
          !!phone && allPersons.some((p) => String(p?.phone || "").trim() === phone);

        if (phoneExists) {
          toast.error(
            "Number kan hore ugu jira System ka"
          );
          return;
        }
      ...
      // ---------- DELETE (REMOVE FROM AGREEMENT ONLY) ----------
      if (operation === "delete") {
        if (!personId) {
          toast.error("Person ID ma jiro");
          return;
